...
class CG_AI_Post_Generator {
    ...
    public function handle_generate_post() {
        ...
        for ( $i = 0; $i < $count; $i++ ) {
            ...
            if ( is_wp_error( $result ) ) {
                // If first fails, bail; if later fails, just note it.
                if ( $i === 0 ) {
        ...
        // Response: keep backward-compatible shape but include all created posts.
    ...
    protected function generate_single_post( $title, $instructions, $post_type, $tone, $api_url, $api_key, $generate_featured_image, $auto_taxonomy ) {
        ...
        if ( json_last_error() !== JSON_ERROR_NONE || ! is_array( $parsed ) ) {
            // Fallback: treat entire content as article, no taxonomy.
            $parsed = [
                'content'    => $raw_content,
                'categories' => [],
                'tags'       => [],
        ...
        // Only apply categories to standard posts (or if taxonomy exists).
        $cat_ids = [];
        ...
        // Tags (for posts; or if post type supports post_tag).
        ...
        // Featured image via OpenAI Images API.
